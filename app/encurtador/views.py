...
from app.encurtador.models import UrlRedirect, UrlLog
...
def relatorios(requisicao, slug):
    ...
    redirecionamentos_por_data = list(
        UrlRedirect.objects.filter(slug=slug).annotate(data=TruncDate('logs__criado_em')).annotate(cliques=Count('data')).order_by('data')
    )
